...
export default function AccountsTab({ currentUserId }: { currentUserId: string | null }) {
  ...
  const deleteAccount = async () => {
    if (!deleteConfirmation || deleteConfirmation.id === currentUserId) return;
    try {
      const res = await fetch("/api/accounts", {
        method: "DELETE",
  ...
  return (
    <section className="glass-card rounded-2xl p-6 shadow-xl">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowModal(true)} className="px-4 py-2.5 rounded-xl bg-gold-600 hover:bg-gold-500 hover:cursor-pointer text-white font-semibold text-sm">
                ...
                <tr key={user.id} className="hover:bg-slate-900/35 transition-colors">
                    ...
                    <td className="py-4.5 px-6 text-right">
                        {user.id !== currentUserId && (
                            <button onClick={() => setDeleteConfirmation({type: 'account', id: user.id})} className="p-1.5 hover:text-rose-400 hover:cursor-pointer transition-colors"><Trash2 className="h-4 w-4" /></button>
                        )}
                    </td>
                </tr>
                ))
            ...
            {!mfaData ? (
                <form onSubmit={createAccount} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        <button type="submit" className="px-4 py-2 rounded-xl bg-gold-600 hover:bg-gold-500 hover:cursor-pointer text-white font-semibold">Next</button>
                    </div>
                </form>
            ) : (
                <div className="space-y-4 text-center">
                    ...
                    <button 
                      onClick={verifyMFA} 
                      disabled={verifying}
                      className={`w-full py-3 rounded-xl bg-gold-600 hover:bg-gold-500 hover:cursor-pointer text-white font-semibold ${verifying ? 'opacity-50 cursor-wait' : ''}`}
                    >
        ...
        <div className="fixed inset-0 z-110 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 h-full w-full">
          <div className="glass-card w-full max-w-sm rounded-2xl p-6 border border-slate-800 text-center">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={deleteAccount} className="flex-1 px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 hover:cursor-pointer text-white text-sm font-semibold">Delete</button>
            </div>
          </div>
        </div>
      )}
